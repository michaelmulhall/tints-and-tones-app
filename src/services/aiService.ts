...
console.log('[Paint Visualizer] Backend URL:', BACKEND_API_URL);
...
export const generatePaintedRoom = async (
  ...
  try {
    ...
    console.log('[Paint Visualizer] Attempting to connect to backend:', BACKEND_API_URL);
    ...
    try {
      ...
      console.error('[Paint Visualizer] Fetch failed:', fetchError);
      throw new Error(
        `Cannot connect to backend server. ${
          BACKEND_API_URL.includes('localhost') 
            ? 'Backend server may not be running. Start it with: npm run server' 
            : 'Backend server is unreachable. Check your VITE_BACKEND_URL environment variable.'
        }`
      );
    }
